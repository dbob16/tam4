<script>
    ...
    let ifPrefix = $derived(Object.keys(currentPrefix).length > 0 ? true : false);
</script>
...
<div id="current_prefix" class="flex flex-col gap-1">
    ...
    <div class={bS[ifPrefix ? currentPrefix.color : "gray"]}>{ifPrefix ? currentPrefix.prefix : "Please select prefix below."}</div>
</div>

<div id="main_grid" class="my-2 flex flex-row flex-wrap gap-4">
    <div id="prefix_selection" class="flex flex-col gap-1 items-center">
        <h2 class="font-bold text-lg">Prefix Selection</h2>
        {#each currentPrefixes as prefix (prefix.prefix)}
        <button class="{bS[prefix.color]} w-full" onclick={() => currentPrefix = {...prefix}}>{prefix.prefix}</button>
        {/each}
    </div>
</div>
